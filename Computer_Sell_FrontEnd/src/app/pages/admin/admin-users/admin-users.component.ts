...
export class AdminUsersComponent implements OnInit {
  ...
  load(): void {
    ...
    this.userService.listUsers({ keyword: this.keyword || undefined, page: this.page, size: this.size, sortBy: this.sortBy }).subscribe({
      next: (res) => {
  ...
  save(){
    ...
    if(this.isEditing){
      ...
      this.userService.createUser(payload).subscribe({ next: (res) => {
